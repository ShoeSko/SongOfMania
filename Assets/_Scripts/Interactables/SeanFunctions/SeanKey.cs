...
public class SeanKey : ObjectBase
{
    [Header("Highlight needs")]
    [SerializeField] private MeshRenderer[] materialsToChange;
    [SerializeField] private Material originalKeyMaterial;
    ...
    {
        base.OnInspect();
        SpokenDisplay.ShowDisplaySpoken_Static(itemInstance.myItemList.item[4].inspectPromt);
    }
        ...
        if (!JuiceToggle.s_juiceHighlight)
        {
            ...
            else if (!isNPC)
            {
                for (int i = 0; i < materialsToChange.Length; i++)
                {
                    ...
                    materialsToChange[i].material = originalKeyMaterial;
                }
            }
        }
    }
